...
async function put(key, data, contentType) {
  await r2.send(new PutObjectCommand({ Bucket: process.env.R2_BUCKET_NAME, Key: key, Body: data, ContentType: contentType, CacheControl: 'public, max-age=31536000' }));
}
...
app.post('/api/sign', upload.fields([{ name: 'ipa', maxCount: 1 }, { name: 'p12', maxCount: 1 }, { name: 'mobileprovision', maxCount: 1 }]), async (req, res) => {
  ...
  const ipa = files.ipa?.[0], p12 = files.p12?.[0], prov = files.mobileprovision?.[0];
  const password = String(req.body.password || '');
  const bundleId = String(req.body.bundleId || process.env.DEFAULT_BUNDLE_ID || 'com.example.app').replace(/[^A-Za-z0-9.-]/g, '');
  ...
  try {
    ...
    if (r2Enabled) {
      ...
      download = `${process.env.R2_PUBLIC_BASE_URL.replace(/\/$/, '')}/${key}`;
    } else {
    ...
    builds.set(id, { file: path.join(ROOT, `${id}.ipa`), name: filename, manifest, key, persistent: r2Enabled });

    const manifestUrl = `${base}/manifest/${id}.plist`;
    res.json({ id, download, manifest: manifestUrl, install: `itms-services://?action=download-manifest&url=${encodeURIComponent(manifestUrl)}`, persistent: r2Enabled });
  } catch (e) {
